...
import ImageCretePopUp from '../ImageCretePopUp /CretePopUp'
...
const ImageInfo = ({ setToggleColumn, title, itemsNumber, isUserListInfo }: ListInfoProp) => {
      ...
      <div className="client-list-info">
        ...
        <div className="admin-user-creation">
          ...
          <Button className={`admin-user-creation-btn`} onClick={() => handleAdd()}>
            {width < 600 ? '+' : isUserListInfo ? 'Create Image' : 'Create Role'}
          </Button>
        </div>
      </div>

      {showPopup && <ImageCretePopUp onClose={closePopup} />}
    </>
  )
}
